...
\noindent La modifica del \textit{database} ha generato una serie di modifiche a cascata a partire dalle classi dei \textit{data access layer} delle \acrog{api} fino a raggiungere le classi del Model nel \textit{frontend}. Tutti i controlli vengono effettuati nel ViewModel, in particolare nella classe \texttt{ViewEditSpesaManager}, ovvero la classe ViewModel relativa alla pagina di modifica/creazione delle spese, ora è presente uno \texttt{switch} nella funzione di salvataggio della spesa appena creata o modificata.
...
Successivamente alla creazione della tabella ho creato tutte le \acrog{api} necessarie e creato le funzioni che calcolano la data corretta in base al parametro all'interno del \textit{frontend}, in particolare nella ViewModel.
